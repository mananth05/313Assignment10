...
class Graph:
    ...
    def compute_depth(self):
        ...
        def dfs_depth(cur):
            if self.vertices[cur].depth != -1:
                return self.vertices[cur].depth

            res = 0
            for x in self.get_adjacent_vertices(cur):
                res = max(res, 1 + dfs_depth(x))
            self.vertices[cur].depth = res
            return res
    ...
    def has_cycle(self):
        ...
        for _ in range(len(self.vertices)):
            ...
            cur_visited.append(False)

        def dfs(cur):
            visited[cur] = True
            cur_visited[cur] = True

            for x in range(len(self.vertices)):
                if self.adjacency_matrix[cur][x] == 1:
                    if not visited[x] and dfs(x):
                        return True
                    if cur_visited[x]:
                        return True

            cur_visited[cur] = False
            return False
    ...
    def get_registration_plan(self):
        ...
        for i in range(len(self.vertices)):
            for x in self.get_adjacent_vertices(i):
                deg[x] += 1
        ...
        while to_take:
            sem = []
            nxt = []
            rng = min(4, len(to_take))
            for _ in range(rng):
                idx = to_take.pop(0)
                sem.append(self.vertices[idx].label)

                for x in self.get_adjacent_vertices(idx):
                    deg[x] -= 1
                    if deg[x] == 0:
                        nxt.append(x)
...
def main():
    ...
    for _ in range(num_edge):
        ...
        from_idx = graph.get_index(from_vtx)
        to_idx = graph.get_index(to_vtx)
